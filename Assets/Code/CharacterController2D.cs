...
public class CharacterController2D : MonoBehaviour 
{
    ...
    public LayerMask PlatformMask; //mask character will collide with
    ...
    public void Jump()
    {
        //TODO: add moving platform support
    ...
    private void Move(Vector2 deltaMovement)
    {
        ...
        if (HandleCollisions)
        {
            ...
            if (deltaMovement.y < 0 && wasGrounded)
                HandleVerticleSlope(ref deltaMovement);

            if (Math.Abs(deltaMovement.x) > .001f)
        ...
        if (StandingOn != null)
        {
            ...
            if (_lastStandingOn != StandingOn)
            {
                if (_lastStandingOn != null)
                    _lastStandingOn.SendMessage("ControllerExitD", this, SendMessageOptions.DontRequireReceiver);
    ...
    private void HandlePlatforms()
    {
        if (StandingOn != null)
        {
            ...
            var moveDistance = newGlobalPlatformPoint - _activeGlobalPlatformPoint; // test, update

            if (moveDistance != Vector3.zero)
                transform.Translate(moveDistance, Space.World);

            PlatformVelocity = (newGlobalPlatformPoint - _activeLocalPlatformPoint)/Time.deltaTime;
        }
        else
    ...
    private void CorrectHorizontalPlacement(ref Vector2 deltaMovement, bool isRight)
    {
        ...
        var rayOrgin = isRight ? _raycastBottomRight : _raycastBottomLeft;

        if (isRight)
            rayOrgin.x -= (halfWidth - SkinWidth);
        else
            rayOrgin.x += (halfWidth - SkinWidth);
    ...
    {
        var size =
            new Vector2(_boxCollider.size.x*Mathf.Abs(_localScale.x), _boxCollider.size.y*Mathf.Abs(_localScale.y))/2;
    ...
    private void MoveVertically(ref Vector2 deltaMovement)
    {
        ...
        var rayOrgin = isGoingUp ? _raycastTopLeft : _raycastBottomLeft;

        rayOrgin.x += deltaMovement.x;
    ...
    private void HandleVerticleSlope(ref Vector2 deltaMovement)
    {
